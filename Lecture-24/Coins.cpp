...
double coins(ll h, ll n) {
	// base case
	if (h == 0) return dp[h][n] = 1;
	if (n == 0) return dp[h][n] = 0;

	if (dp[h][n] > -0.9) return dp[h][n];

	return dp[h][n] = p[n] * coins(h - 1, n - 1) + (1 - p[n]) * coins(h, n - 1);
}
...
void solve() {
	...
	cout << setprecision(10) << coins((n + 1) / 2, n) << endl;
